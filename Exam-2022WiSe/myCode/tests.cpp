...
void bankTests () {
     ...
     */
    auto cus= bank.allCustomers();
    assertTrue(cus.size() == 3, "Not three customers");


   /*
    * (2) Assert that the customer with id 0000000002 can be looked up.
    */
    assertTrue(bank.lookupCustomer("0000000002") != nullptr, "Cannot be looked up");

   /*
    * (3) Assert that there is no customer with id 0000000042.
    */
    assertTrue(bank.lookupCustomer("0000000042") != nullptr, "Customer found");
...
void accountTests () {
     ...
     */
    assertTrue(bank.lookupCustomer("0000000001")->getFirstName() == "Alice", "Alice not found");


    /*
     * (2) Assert that Alice has 5 accounts.
     */
    auto al= bank.lookupCustomer("0000000001");
    int nAcc = 0;
    if(al->getFirstName() == "Alice"){
        nAcc = al->allAccounts().size();
    }
    assertTrue(nAcc == 5, "Alice doesnot have 5 accounts");
